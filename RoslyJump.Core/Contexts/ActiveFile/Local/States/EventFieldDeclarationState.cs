...
{
    public class EventFieldDeclarationState : ClassMemberStateBase<EventFieldDeclarationSyntax>
    {
        ...
        protected override CombinedSyntaxNode[] QueryTargetNodesFunc()
        {
            CombinedSyntaxNode[]? nodes = this.BaseNode.Parent
                ?.ChildNodes()
                ?.Where(x => x.GetType() == typeof(EventFieldDeclarationSyntax))
            ...
            if (nodes == null || nodes.IsNullOrEmpty())
            {
                throw new InvalidOperationException(
                    $"Unable to query target nodes for {nameof(EventFieldDeclarationState)}.");
            }
