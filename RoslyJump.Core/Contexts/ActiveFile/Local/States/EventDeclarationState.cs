...
{
    public class EventDeclarationState : PropOrEventClassMemberStateBase<EventDeclarationSyntax>
    {
        ...
        {
            CombinedSyntaxNode[]? nodes = BaseNode.Parent
                ?.ChildNodes()
                ?.Where(x => x.GetType() == typeof(EventDeclarationSyntax))
            ...
            if (nodes == null || nodes.IsNullOrEmpty())
            {
                throw new InvalidOperationException(
                    $"Unable to query target nodes for {nameof(EventDeclarationState)}.");
            }
